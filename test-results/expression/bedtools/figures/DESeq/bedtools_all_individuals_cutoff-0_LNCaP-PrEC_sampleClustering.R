...
vsd = read.delim('/Users/olgabotvinnik/workspace/rna-seq-diff-exprn/test-results/expression/bedtools/figures/DESeq/bedtools_all_individuals_cutoff-0_vsd.txt', row.names=1)

# Get the Euclidean distance between samples
dists = dist(t(vsd))

# Labels for the rows based on the sample conditions, e.g. their treatment groups
labRow = c('LNCaP','LNCaP','LNCaP','LNCaP','PrEC','PrEC')

# Open a pdf file to put the graphics we're about to create
pdf('/Users/olgabotvinnik/workspace/rna-seq-diff-exprn/test-results/expression/bedtools/figures/DESeq/bedtools_all_individuals_cutoff-0_LNCaP-PrEC_sampleClustering.pdf')
