...
# Example run: qsub -o test/test.out -e test/test.err map-reads.sh run2/Sample_3/trimmed_3_TAGCTT_L006_R1_001.fastq run2/Sample_3/trimmed_3_TAGCTT_L006_R2_001.fastq test/test test
...
# Do the mapping using Bowtie to determine mappability and insert size
/share/apps/bowtie-0.12.7/bowtie --chunkmbs 256 -S \
    -m 1 --maxins 500 -p4 --phred33-quals \
    --un $UNALIGNED -1 $READ_ONE -2 $READ_TWO hg19 | \
    /share/apps/samtools-0.1.18/samtools view -F 4 \
    -S -b - > $BAM
...
# Get mean and standard deviation of  read insert size from Bowtie
READ_STATS=$BAM_PREFIX\_read_stats.txt
/share/apps/samtools-0.1.18/samtools view -f66 $BAM | \
    awk '{sumsq+=$9*$9; sum+=$9<0?-$9:$9} END {print int(sqrt(sumsq/NR - (sum/NR)**2)); print int(sum/NR)}' > $READ_STATS

INSERT_SIZE_MEAN=`sed -n '2p;d' $READ_STATS`
INSERT_SIZE_SD=`sed -n '1p;d' $READ_STATS`

echo 'read stats - mean:' $INSERT_SIZE_MEAN \
'  std dev:' $INSERT_SIZE_SD
...
#/share/apps/tophat-1.4.1.Linux_x86_64/tophat --num-threads 4 -G $GTF \
#    --mate-inner-dist $INSERT_SIZE_MEAN --mate-std-dev $INSERT_SIZE_SD \
#    --library-type fr-unstranded -o $OUT_DIR /hg19 $READ_ONE $READ_TWO
