...
# NUM_GFFs is assumed to be the same as the number of BEDs,
# since the same comparison will be done for all but with
# different programs
NUM_GFFs=`echo $GFFs | awk -F"," '{print NF}'`

for i in {1..$NUM_GFFs}
do
    THIS_GFF=`echo $GFFs | awk -F"," -v i=$i '{print $i}'`
    THIS_BED=`echo $BEDs | awk -F"," -v i=$i '{print $i}'`
    THIS_SUFFIX=`echo $SUFFIXES | awk -F"," -v i=$i '{print $i}'`
    THIS_BAM=$BAM_PREFIX\_$THIS_SUFFIX.bam
    samtools view -b $BAM | \
...
bedtools coverage \
    $coverageBed_options \
    -abam $THIS_BAM \
    -b $THIS_BED >$REF_BEDTOOLS_PREFIX.txt


# Turn bedtools counts files into circos data
    awk '{ print $1"\t"$2"\t"$3"\t"$13"\tgene="$4}' \
	$SINO_BEDTOOLS_PREFIX.txt > \
	$SINO_BEDTOOLS_PREFIX.circos
    awk '{ print $1"\t"$2"\t"$3"\t"$13"\tgene="$4}' \
...
THIS_READ_SORTED=$THIS_BAM_PREFIX\_sorted_read_name.sam
samtools view $THIS_BAM | \
    sort -s -k 1,1 >$THIS_READ_SORTED
...
R --slave --vanilla --quiet --no-save --args \
    $BEDTOOLS_PREFIX.circos $INTERVAL \
    $BEDTOOLS_PREFIX.circos.$INTERVAL \
    $AVG_METHOD
